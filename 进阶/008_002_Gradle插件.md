...
1.在根目录下新建`buildSrc`目录，新建目录`src/main/groovy`，再新建package，为`com/imooc/router/gradle`
...
新建一个工程`GradlePluginDemo`

1.在根目录下的`build.gradle`指定本地maven仓库的位置

2.声明依赖的插件

![063](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/063.png)

3.在app模块下的`build.gradle`，应用插件，配置参数

![064](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/064.png)



build工程，或者执行`./gradlew clean -q`，控制台有如下的输出：

![065](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/065.png)
...
在上面的例子中有新建`META-INF/`文件夹

参考：

+ [关于java的服务提供发现机制](https://www.modb.pro/db/231287)

> SPI即Service Provider Interface，是JDK自带的一种服务提供发现机制，可以用来启用框架扩展和替换组件，主要是被框架的开发人员使用，比如java.sql.Driver接口，不同厂商可以针对同一接口做出不同的实现，MySQL和PostgreSQL都有不同的实现提供给用户，而Java的SPI机制可以为某个接口寻找服务实现。Java中SPI机制主要思想是将装配的控制权移到程序之外，在模块化设计中这个机制尤其重要，其核心思想就是解耦。
>
> ![066](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/066.png)
>
