...
> **注解处理程序的选择**
>
> 注解处理程序，负责在源码编译期间，收集所有的注解，并回调给对应的注解处理器。
>
> - 若工程**未使用**Kotlin，那么使用Android原生提供的`annotationProcessor`程序声明注解处理器即可；
> - 若工程**使用了**Kotlin，需要使用kapt来声明注解处理器，kapt会收集处理Kotlin和Java中所有注解；需要注意的是，工程中若之前使用过 annotationProcessor，就需要**全部替换**为kapt关键字，二者不能混用，否则，使用annotationProcessor声明的那部分注解处理器**将不会被回调**（因为注解处理流程已经被kapt完全接管了）。
>
> 综上，通常情况下，选择**kapt**即可，这样后续引入Kotlin也不需要做过多改动
>
> **向注解处理器传递参数**
>
> 以kapt为例，向注解处理器传递参数的方式为：
>
> ```groovy
> kapt {
>     arguments {
...
在gradle插件模块完成汇总生成文档功能，`RouterPlugin`

![092](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/092.png)

![090](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/090.png)

![091](https://github.com/winfredzen/Android-Basic/blob/master/%E8%BF%9B%E9%98%B6/image/091.png)
