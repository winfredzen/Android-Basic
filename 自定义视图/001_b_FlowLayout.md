...
该例子来源于教程，存在某些bug，但对于学习自定义`ViewGroup`是不错的例子
...
1.在调用`child.getMeasuredWidth()`、`child.getMeasuredHeight()`之前，一定先要调用`measureChild(child,widthMeasureSpec,heightMeasureSpec);`

> 在调用`onMeasure()`函数之后才能调用`getMeasuredWidth()`
>
> 同样，在调用`onLayout()`函数后，`getWidth()`函数才能获取值
...
public class FlowLayout extends ViewGroup {
    ...
    // 重写onMeasure函数，计算当前FlowLayout所占区域大小
    @Override
